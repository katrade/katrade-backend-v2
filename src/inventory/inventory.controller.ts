...
export class InventoryController {
  ...
  async getMatchInventory(@Req() req, @Query('id') inventoryId: string) {
    return await this.inventoryService.getMatchInventory(
      req.user.uid,
      inventoryId,
    );
  }
  ...
  async search(@Req() req, @Query('query') query: string) {
    let search: Inventory[] = await this.inventoryService.searchInventory(
      req.user.uid,
      query,
    );
    return search;
  ...
  async changePic(@Body() body: any) {
    return await this.inventoryService.changeInventoryPic(
      body.id,
      body.pictures,
    );
  }
