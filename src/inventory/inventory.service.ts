...
export class InventoryService {
  ...
  async findInventoryById(
    inventoryId: string,
  ): Promise<ResponseInventory | any> {
    const i: Inventory = await this.inventoryModel.findOne({
      _id: inventoryId,
    });
    return i;
  }
  ...
  async getAll() {
    ...
    const res = inventory.sort(
      (a, b) => b.favourite.length - a.favourite.length,
    );
    return res;
  }

  async getUserInventory(userId: string) {
    let allUserInventory: Inventory[] = await this.inventoryModel.find({
      owner: userId,
    });
  ...
  async getSelectItem(uid: string) {
    const userInventory = await this.inventoryModel.find({
      owner: uid,
      lock: 0,
    });
  ...
  async getMatchInventory(uid: string, inventoryId: string) {
    ...
    for (let i = 0; i < userInventory.length; i++) {
      for (let j = 0; j < inventory.require.length; j++) {
        if (
          userInventory[i].category.childCategoryEn ===
            inventory.require[j].reqCat.childCategoryEn &&
          userInventory[i].category.parentCategoryEn ===
            inventory.require[j].reqCat.parentCategoryEn
        ) {
  ...
  async getMatch(uid: string) {
    const userInventory: Inventory[] = await this.inventoryModel.find({
      owner: uid,
    });
    if (!userInventory) {
    ...
    const match: Inventory[] = await this.inventoryModel.find({
      owner: { $nin: [uid] },
      category: { $in: requireCategory },
    });
    ...
    for (let i = 0; i < match.length; i++) {
      ...
      let tmp: Inventory[] = await this.inventoryModel.find({
        owner: uid,
        category: { $in: requireCategory },
      });
      if (tmp.length > 0) {
  ...
  async newInv(payload: any, thing: Inventory): Promise<{ id: string }> {
    ...
    await this.inventoryModel.create(newThing).then(async (response) => {
      ...
      await this.userModel
        .updateOne(
          { _id: payload.uid },
          { $push: { inventories: [response._id.toString()] } },
        )
        .then(() => {
  ...
  async searchByCategory(uid: string, category: any) {
    if (!category.childCategoryEn) {
      return await this.inventoryModel.find({
        owner: { $ne: uid },
        'category.parentCategoryEn': category.parentCategoryEn,
      });
    } else {
      return await this.inventoryModel.find({
        owner: { $ne: uid },
        'category.parentCategoryEn': category.parentCategoryEn,
        'category.childCategoryEn': category.childCategoryEn,
      });
    }
  }
